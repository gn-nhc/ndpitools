/* ndpi2tiff
 v. 1.5
 Copyright (c) 2011-2013 Christophe Deroulers
...
static char* build_outfilename(const char* infilename)
{
	...
	if ((infilename[l-1] == 'i' || infilename[l-1] == 'I') &&
	    ...
	    (infilename[l-5] == '.'))
		l-= 5;
	outfilename = _TIFFmalloc(l+sizeof(TIFF_SUFFIX));
	if (outfilename == NULL)
	{
		perror("Insufficient memory for a character string ");
		exit(EXIT_FAILURE);
...
usage(void)
{
	...
	setbuf(stderr, buf);
	fprintf(stderr, "ndpi2tiff version 1.5-2 license GNU GPL v3 (c) 2011-2013 Christophe Deroulers\n"
...
tiffcp(TIFF* in, TIFF* out)
{
	...
	if (! TIFFGetField(in, NDPITAG_MAGNIFICATION, &ndpi_magnification) ) {
	...
	if (ndpi_magnification == -1) {
		...
		if (! TIFFGetField(in, NDPITAG_ZOFFSET, &ndpi_zoffset)) {
			TIFFError(TIFFFileName(in),
	...
	TIFFSetField(out, TIFFTAG_IMAGEDESCRIPTION, imagedescription);
	_TIFFfree(imagedescription);
	/*
...
static int x(TIFF* in, \
    uint8* buf, uint32 firstrow, uint32 lengthtoread, uint32 imagewidth, tsample_t spp)
typedef int (*readFunc)(TIFF*, uint8*, uint32, uint32, uint32, tsample_t);

#define	DECLAREwriteFunc(x) \
static int x(TIFF* out, \
    uint8* buf, uint32 firstrow, uint32 lengthtowrite, uint32 imagewidth, tsample_t spp)
typedef int (*writeFunc)(TIFF*, uint8*, uint32, uint32, uint32, tsample_t);
	...
	uint32 imagelength, uint32 imagewidth, tsample_t spp)
{
	...
	tsize_t bufferlength = tilelength == (uint32) -1 ? imagelength : tilelength;
...
DECLAREreadFunc(readContigTilesIntoBuffer)
{
	...
	for (row = firstrow; row < firstrow + lengthtoread; row += tl) {
		uint32 nrow = (row+tl > firstrow+lengthtoread) ?
...
DECLAREwriteFunc(writeBufferToContigStrips)
{
	...
	for (row = firstrow; row < firstrow+lengthtowrite; row += rowsperstrip) {
		uint32 nrows = (row+rowsperstrip > firstrow+lengthtowrite) ?
...
DECLAREwriteFunc(writeBufferToContigTiles)
{
	...
	for (row = firstrow; row < firstrow+lengthtowrite; row += tilelength) {
		uint32 nrow = (row+tl > firstrow+lengthtowrite) ?
...
DECLAREwriteFunc(writeBufferToSeparateTiles)
{
	...
	for (row = firstrow; row < firstrow+lengthtowrite; row += tl) {
		uint32 nrow = (row+tl > firstrow+lengthtowrite) ? firstrow+lengthtowrite-row : tl;
		uint32 colb = 0;
		uint32 col;
