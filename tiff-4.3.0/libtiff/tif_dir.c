...
_TIFFVSetField(TIFF* tif, uint32_t tag, va_list ap)
{
	...
	case TIFFTAG_INKNAMES:
		...
		break;
	case NDPITAG_MAGNIFICATION:
		td->td_ndpimagnification = (float) va_arg(ap, double);
		break;
	case NDPITAG_ZOFFSET:
		td->td_ndpizoffset = (int32_t) va_arg(ap, int32_t);
		break;
	case NDPITAG_USERGIVENSLIDELABEL:
		s = (char*) va_arg(ap, char*);
		_TIFFsetString(&td->td_ndpiusergivenslidelabel, s);
		break;
	case NDPITAG_BLANKLANES:
		td->td_ndpinblanklanes = va_arg(ap, uint32_t);
		_TIFFsetLongArray(&td->td_ndpiblanklanes, va_arg(ap, uint32_t*), (long) td->td_ndpinblanklanes);
		break;
	case NDPITAG_COMMENTS:
		s = (char*) va_arg(ap, char*);
		_TIFFsetString(&td->td_ndpicomments, s);
		break;
	case NDPITAG_FLUORESCENCE:
		s = (char*) va_arg(ap, char*);
		_TIFFsetString(&td->td_ndpifluorescence, s);
		break;
	...
	switch (standard_tag) {
		...
		case TIFFTAG_INKNAMES:
			*va_arg(ap, const char**) = td->td_inknames;
			break;
		case NDPITAG_MAGNIFICATION:
			*va_arg(ap, float*) = td->td_ndpimagnification;
			break;
		case NDPITAG_ZOFFSET:
			*va_arg(ap, int32_t*) = td->td_ndpizoffset;
			break;
		case NDPITAG_USERGIVENSLIDELABEL:
			*va_arg(ap, char**) = td->td_ndpiusergivenslidelabel;
			break;
		case NDPITAG_BLANKLANES:
			*va_arg(ap, uint32_t*) = td->td_ndpinblanklanes;
			*va_arg(ap, uint32_t**) = td->td_ndpiblanklanes;
			break;
		case NDPITAG_COMMENTS:
			*va_arg(ap, char**) = td->td_ndpicomments;
			break;
		case NDPITAG_FLUORESCENCE:
			*va_arg(ap, char**) = td->td_ndpifluorescence;
			break;
		default:
			{
				int i;
