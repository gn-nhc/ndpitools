...
static uint64 NDPIFixOffset(uint32 dataoff32, uint64 diroff, int swab);
static void NDPIFixOffsets(uint64 * p_dataoff, uint64 diroff, uint32 count);
		...
		switch (dp->tdir_tag)
		{
			...
			case TIFFTAG_TILEOFFSETS:
				...
				if (!TIFFFetchStripThing(tif,dp,tif->tif_dir.td_nstrips,&tif->tif_dir.td_stripoffset))  
					goto bad;
				NDPIFixOffsets(tif->tif_dir.td_stripoffset,
				    tif->tif_diroff, tif->tif_dir.td_nstrips); /* toto */
#endif                                
				break;
			case TIFFTAG_STRIPBYTECOUNTS:
			case TIFFTAG_TILEBYTECOUNTS:
...
static uint64
NDPIFixOffset(uint32 dataoff32, uint64 diroff, int swab)
{
	uint64 dataoff64;
	if (swab)
	...
	for (n=0; n<dircount16; n++)
	{
		...
		if (!(tif->tif_flags&TIFF_BIGTIFF))
		{
			...
			mb->tdir_offset.toff_long8=
			    NDPIFixOffset(*(uint32*)ma, diroff, tif->tif_flags&TIFF_SWAB);
			ma+=sizeof(uint32);
		}
		else
		{
...
static void
NDPIFixOffsets(uint64 * p_dataoff, uint64 diroff, uint32 count)
{
	uint32 u;
	uint64 c = ((diroff >> 32) << 32);
...
int _TIFFFillStriles( TIFF *tif )
{
	...
	{
		NDPIFixOffsets(td->td_stripoffset, tif->tif_diroff, td->td_nstrips); /* toto */
	}
