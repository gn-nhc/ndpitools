...
_TIFFVSetField(TIFF* tif, uint32 tag, va_list ap)
{
	...
	case NDPITAG_ZOFFSET:
		td->td_ndpizoffset = (int32) va_arg(ap, int32);
		break;
	...
	case NDPITAG_BLANKLANES:
		td->td_ndpinblanklanes = va_arg(ap, uint32);
		_TIFFsetLongArray(&td->td_ndpiblanklanes, va_arg(ap, uint32*), (long) td->td_ndpinblanklanes);
		break;
	...
	switch (standard_tag) {
		...
		case NDPITAG_MAGNIFICATION:
			*va_arg(ap, float*) = td->td_ndpimagnification;
			break;
		case NDPITAG_ZOFFSET:
			*va_arg(ap, int32*) = td->td_ndpizoffset;
			break;
		case NDPITAG_USERGIVENSLIDELABEL:
			*va_arg(ap, char**) = td->td_ndpiusergivenslidelabel;
			break;
		case NDPITAG_BLANKLANES:
			*va_arg(ap, uint32*) = td->td_ndpinblanklanes;
			*va_arg(ap, uint32**) = td->td_ndpiblanklanes;
			break;
		case NDPITAG_COMMENTS:
