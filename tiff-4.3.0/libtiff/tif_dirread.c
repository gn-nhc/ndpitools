...
static uint64_t NDPIFixOffset(uint32_t dataoff32, uint64_t diroff, int swab);
static void NDPIFixOffsets(uint64_t * p_dataoff, uint64_t diroff, uint32_t count);
	...
	if (!(tif->tif_flags&TIFF_BIGTIFF))
	{
		...
		else
		{
			...
			if( isMapped(tif) )
				...
				err=TIFFReadDirEntryDataAndRealloc(tif, offset, (tmsize_t)datasize, &data);
			if (err!=TIFFReadDirEntryErrOk)
			{
				_TIFFfree(data);
				return(err);
...
/* END REV 4.0 COMPATIBILITY */
		...
		NDPIFixOffsets(tif->tif_dir.td_stripoffset_p,
		    tif->tif_diroff, tif->tif_dir.td_nstrips);
            }
...
static uint64_t
NDPIFixOffset(uint32_t dataoff32, uint64_t diroff, int swab)
{
	uint64_t dataoff64;
	if (swab)
		TIFFSwabLong(&dataoff32);
	dataoff64 = dataoff32 + ((diroff >> 32) << 32);
	if (dataoff64 >= diroff && dataoff64 >= 0x100000000ULL)
		dataoff64 -= 0x100000000ULL;
	if (swab)
		TIFFSwabLong8(&dataoff64);
	return(dataoff64);
}
	...
	for (n=0; n<dircount16; n++)
	{
		...
		if (!(tif->tif_flags&TIFF_BIGTIFF))
		{
			...
			mb->tdir_offset.toff_long8=
			    NDPIFixOffset(*(uint32_t*)ma, diroff, tif->tif_flags&TIFF_SWAB);
			ma+=sizeof(uint32_t);
		}
		else
		{
...
static void
NDPIFixOffsets(uint64_t * p_dataoff, uint64_t diroff, uint32_t count)
{
	uint32_t u;
	uint64_t c = ((diroff >> 32) << 32);

	for (u = 0 ; u < count ; u++, p_dataoff++) {
		*p_dataoff += c;
		if (*p_dataoff >= diroff && *p_dataoff >= 0x100000000ULL)
			*p_dataoff -= 0x100000000ULL;
	}
}
...
ChopUpSingleUncompressedStrip(TIFF* tif)
{
	...
	{
		NDPIFixOffsets(td->td_stripoffset_p, tif->tif_diroff, td->td_nstrips);
	}
